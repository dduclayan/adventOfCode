...
func findInvalidIDs(tok string) (int, error) {
	...
	for i := start; i <= end; i++ {
		...
		// case 2: total len multiple of 3, at least two groups of three consecutively match
		if len(s)%3 == 0 {
			var substrs []string
			for i := 0; i < len(s); i += 3 {
				substrs = append(substrs, s[i:i+3])
			}
			log.Printf("%v", substrs)

			matchedCount := 0
			initialChunk := substrs[0]
			for idx := range substrs {
				if initialChunk == substrs[idx] {
					matchedCount++
				}
			}
			if matchedCount == len(substrs) && len(substrs) >= 2 {
		...
		if len(s)%2 == 0 {
			...
			if i < 100 {
				...
				if substrs[0] == substrs[1] {
					sum += i
					log.Printf("found match with %v", i)
				}
				continue
			}
			...
			var substrs []string
			for i := 0; i < len(s); i += 2 {
				substrs = append(substrs, s[i:i+2])
			}
			log.Printf("%v", substrs)
